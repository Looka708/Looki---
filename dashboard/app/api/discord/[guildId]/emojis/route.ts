...
export async function GET(_request: Request, { params }: { params: Promise<{ guildId: string }> }) {
  ...
  try {
    const data: any = await fetchDiscord(`https://discord.com/api/v10/guilds/${guildId}/emojis`);
    return NextResponse.json({ emojis: data ?? [] });
  } catch (err: any) {
    return NextResponse.json({ error: err?.message ?? 'Failed to fetch emojis' }, { status: 500 });
  }
}
