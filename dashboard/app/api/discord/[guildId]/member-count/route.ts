...
export async function GET(_request: Request, { params }: { params: Promise<{ guildId: string }> }) {
  ...
  try {
    ...
    const approximate = data?.approximate_member_count ?? data?.member_count ?? 0;
    return NextResponse.json({ totalMembers: approximate, name: data?.name ?? 'Unknown' });
  } catch (err: any) {
    return NextResponse.json({ error: err?.message ?? 'Failed to fetch member count' }, { status: 500 });
  }
}
